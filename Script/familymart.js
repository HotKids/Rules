...
const getCookieRegex = /^https?:\/\/fmapp\.chinafamilymart\.com\.cn\/api\/app\/market\/member\/sign\/sign/;
  ...
  return new Promise((resolve, reject)=>{
    let options = {
      ...
      headers: {
        ...
        "User-Agent": "Fa",
        "deviceId": "DA14D851-2E39-8E3D-B7FD-38710CABAC76",
        "fmVersion": "1.2.3",
        "loginChannel": "app",
        "token": cookie
      },
...
;(async()=>{
  if (magicJS.isRequest && getCookieRegex.test(magicJS.request.url)){
    ...
    if (!!!cookie){
      ...
      let [getCodeErr, signCode] =  await magicJS.attempt(magicJS.retry(GetSignCode, 3, 1000)(cookie), "");
      if (getCodeErr){
        subTitle = getCodeErr;
      }
      else{
        magicJS.log(`当前获取的SignCode：${signCode}`);
        let [checkInErr,[todayRewardNum, signCount, resWordUp, resWordDown, message]] = await magicJS.attempt(magicJS.retry(CheckIn, 3, 1000)(cookie, signCode), [null, null, null, null, ""]);
        if (checkInErr){
          subTitle = checkInErr;
        }
        else if (!!message){
