...
| 文件 / 目录 | 维护方式 | 说明 |
|---|---|---|
| `Sample.conf` | 🤖 自动 | Quantumult X 完整示例配置，由 `sync-config.py` 从 `Surge/Profile.conf` 生成，勿手改 |
| `X/Filter/` | 🤖 自动 | QX 格式规则集，由 `sync-rules.py` 从 `Surge/RULE-SET/` 转换（不支持的规则类型自动跳过），勿手改 |
