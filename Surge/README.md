...
| `Balloon.lcf` | 🤖 自动 | Loon 配置，由 `sync-config.py` 生成，勿手改 |
| `Surfboard.conf` | 🤖 自动 | Surfboard 配置，由 `sync-config.py` 生成，勿手改 |
| `Module/` | 混合 | sgmodule 模块，见下 |

## Module/

- **`BlockAds.sgmodule`** — 🤖 由 `sync-modules.py` 按 `sync-modules.txt` 的上游 URL
  列表聚合生成（按 section 合并、拼音排序、生成 `#!arguments` 逐 App 开关），勿手改；
